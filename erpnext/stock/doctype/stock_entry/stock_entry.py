...
class StockEntry(StockController):
	...
	def check_user_permision(self):
		user = frappe.session.user
		role = frappe.get_roles(user)
		if 'System Manager' in role or 'Stock Manager' in role or 'Administrator' in role:
			return
		allow = 'Warehouse'
		for item in self.get("items"):
			for_value = str(item.t_warehouse)
			user_permision = frappe.db.sql("""select name from `tabUser Permission`
					 where user='{}' and  allow = '{}' and for_value = '{}'""".format(user, allow, for_value), as_dict=True)
			if not user_permision:
				frappe.throw(_("You Don't Have Permission On Target Warehouse {0} ").format(for_value))
		return
